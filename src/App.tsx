...
// ... (Constants & Helpers remain the same) ...
...
export default function App() {
  ...
  const [owners, setOwners] = useState(() => JSON.parse(localStorage.getItem('app_owners')) || ['Shared', 'Randy', 'Sam']);
  const [categories, setCategories] = useState(() => JSON.parse(localStorage.getItem('app_categories')) || DEFAULT_CATEGORIES);

  const [currentDate, setCurrentDate] = useState(new Date()); 
  const monthKey = `${currentDate.getFullYear()}-${String(currentDate.getMonth() + 1).padStart(2, '0')}`;
  const monthLabel = currentDate.toLocaleString('default', { month: 'long', year: 'numeric' });

  const [appStartDate, setAppStartDate] = useState(() => localStorage.getItem('app_start_date') || monthKey);
  const [startingBalances, setStartingBalances] = useState(() => JSON.parse(localStorage.getItem('app_starting_balances')) || {});

  const [recurringBills, setRecurringBills] = useState(() => JSON.parse(localStorage.getItem('app_recurring')) || []);
  const [savingsGoals, setSavingsGoals] = useState(() => JSON.parse(localStorage.getItem('app_goals')) || []);
  const [monthlyData, setMonthlyData] = useState(() => JSON.parse(localStorage.getItem('app_months')) || {});
  ...
  useEffect(() => { if (!monthlyData[monthKey]) { const clonedBills = recurringBills.map(b => ({ ...b, snapshotId: `${monthKey}-${b.id}-${Math.random().toString(36).substring(7)}`, isPaid: false, originalOwner: b.owner, category: b.category || 'other' })); const activeGoals = savingsGoals.filter(g => g.totalPaid < g.target); const clonedGoals = activeGoals.map(g => ({ id: `goal-${g.id}`, snapshotId: `${monthKey}-goal-${g.id}-${Math.random().toString(36).substring(7)}`, name: `🎯 ${g.name}`, amount: g.monthlyMin, dueDate: 28, columnId: 'pay2', owner: g.owner, isPaid: false, isSavings: true, goalId: g.id, originalOwner: g.owner, category: 'savings' })); setMonthlyData(prev => ({ ...prev, [monthKey]: { bills: [...clonedBills, ...clonedGoals], incomes: {}, todos: [] } })); } }, [monthKey, recurringBills, savingsGoals, monthlyData]);
  useEffect(() => { localStorage.setItem('app_theme', theme); document.body.setAttribute('data-theme', theme); }, [theme]);
  useEffect(() => { localStorage.setItem('app_owners', JSON.stringify(owners)); }, [owners]);
  useEffect(() => { localStorage.setItem('app_categories', JSON.stringify(categories)); }, [categories]);
  useEffect(() => { localStorage.setItem('app_start_date', appStartDate); }, [appStartDate]);
  useEffect(() => { localStorage.setItem('app_starting_balances', JSON.stringify(startingBalances)); }, [startingBalances]);
  useEffect(() => { localStorage.setItem('app_recurring', JSON.stringify(recurringBills)); }, [recurringBills]);
  useEffect(() => { localStorage.setItem('app_goals', JSON.stringify(savingsGoals)); }, [savingsGoals]);
  useEffect(() => { localStorage.setItem('app_months', JSON.stringify(monthlyData)); }, [monthlyData]);
  ...
  const updateCurrentMonth = (newBills, newIncomes, newTodos) => {
    setMonthlyData(prev => {
      ...
      return { ...prev, [monthKey]: { 
        ...currentObj, 
        bills: newBills !== undefined ? newBills : currentObj.bills, 
        incomes: newIncomes !== undefined ? newIncomes : currentObj.incomes,
        todos: newTodos !== undefined ? newTodos : (currentObj.todos || [])
      }};
    });
  };
  ...
  const getRollover = (targetMonthKey, person) => {
    ...
    for(let i=0; i < 120; i++) { 
       ...
       const oldSharedP1 = b.filter(x => x.owner === 'Shared' && x.columnId === 'pay1').reduce((s, x) => s + x.amount, 0);
       const oldSharedP2 = b.filter(x => x.owner === 'Shared' && x.columnId === 'pay2').reduce((s, x) => s + x.amount, 0);
       ...
       const oldPersP1 = b.filter(x => x.owner === person && x.columnId === 'pay1').reduce((s, x) => s + x.amount, 0);
       const oldPersP2 = b.filter(x => x.owner === person && x.columnId === 'pay2').reduce((s, x) => s + x.amount, 0);
  ...
  // ... (Chart Generators remain unchanged) ...
  ...
  // --- ACTIONS: Closing Ritual ---
  const openClosingModal = () => {
    const initial = {};
    owners.filter(o => o !== 'Shared').forEach(o => { initial[o] = getPersonStats(o).totalFree; });
    setClosingBalances(initial);
    setIsClosingOpen(true);
  };

  const handleCloseBooks = (e) => {
    e.preventDefault();
    setMonthlyData(prev => ({
      ...prev,
      [monthKey]: { ...prev[monthKey], closingBalances: closingBalances }
    }));
    setIsClosingOpen(false);
  };

  // NEW: Re-open Books (Unlocks the month)
  const handleReopenBooks = () => {
    if (window.confirm("Re-opening will revert balances to their calculated values. Are you sure?")) {
      setMonthlyData(prev => {
        const updatedMonth = { ...prev[monthKey] };
        delete updatedMonth.closingBalances; // Remove the lock
        return { ...prev, [monthKey]: updatedMonth };
      });
    }
  };

  // ... (Existing actions remain same) ...
          ...
          <div className="animate-fade-in">
              ...
              <div style={{width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                <div>
                  <h1 style={{fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: 10}}>
                    Budget Snapshot
                    {isMonthClosed && <span className="closed-badge"><Lock size={12}/> Closed</span>}
                  </h1>
                  <p className="subtitle">Managing {monthLabel}</p>
                </div>
                <div style={{display: 'flex', gap: '10px'}}>
                  {/* BUTTON SWAP LOGIC */}
                  {!isMonthClosed ? (
                    <button className="btn-close-books" onClick={openClosingModal}>
                      <CheckSquare size={16} /> Close Books
                    </button>
                  ) : (
                    <button className="btn-reopen-books" onClick={handleReopenBooks}>
                      <Unlock size={16} /> Re-open Books
                    </button>
                  )}
                  ...
                  <button className="btn-balance-all" onClick={handleBalanceEverything}><RefreshCw size={18} /> Balance All</button>
                </div>
              </div>
            ...
            {dashboardMode === 'charts' ? (
              <div className="charts-grid animate-fade-in">
                {/* ... Charts ... */}
                <div className="chart-card"><div className="chart-header-row"><h3>Spending Breakdown</h3><div style={{display:'flex', gap:8}}><div className="select-wrapper-small" style={{marginRight: 4}}><Layers size={14} className="icon"/><select value={chartGroupBy} onChange={(e) => setChartGroupBy(e.target.value)}><option value="owner">By Owner</option><option value="category">By Category</option></select></div><div className="select-wrapper-small"><Filter size={14} className="icon"/><select value={breakdownFilter} onChange={(e) => setBreakdownFilter(e.target.value)}><option value="All">All Owners</option>{owners.filter(o => o !== 'Shared').map(o => <option key={o} value={o}>{o}</option>)}</select></div><button className="btn-icon-only" onClick={() => setExpandedChart('breakdown')}><Maximize2 size={16}/></button></div></div><div style={{ width: '100%', height: 300 }}>{renderChartContent('breakdown')}</div></div>
                <div className="chart-card"><div className="chart-header-row"><h3>Money In vs Money Out</h3><div style={{display:'flex', gap:8}}><div className="select-wrapper-small"><Filter size={14} className="icon"/><select value={cashFlowFilter} onChange={(e) => setCashFlowFilter(e.target.value)}><option value="All">Household</option>{owners.filter(o => o !== 'Shared').map(o => <option key={o} value={o}>{o}</option>)}</select></div><button className="btn-icon-only" onClick={() => setExpandedChart('cashflow')}><Maximize2 size={16}/></button></div></div><div style={{ width: '100%', height: 300 }}>{renderChartContent('cashflow')}</div></div>
                <div className="chart-card"><div className="chart-header-row"><h3>Fixed Bills vs Free Cash</h3><button className="btn-icon-only" onClick={() => setExpandedChart('burden')}><Maximize2 size={16}/></button></div><div style={{ width: '100%', height: 300 }}>{renderChartContent('burden')}</div></div>
                <div className="chart-card"><div className="chart-header-row"><h3>6-Month Financial Trend</h3><button className="btn-icon-only" onClick={() => setExpandedChart('trend')}><Maximize2 size={16}/></button></div><div style={{ width: '100%', height: 300 }}>{renderChartContent('trend')}</div></div>
              </div>
            ) : (
              <>
          ...
          <div className="modal-card animate-fade-in" style={{maxWidth: 500}}>
            <div className="modal-header">
              <h3 style={{margin: 0, fontSize: '1.2rem', display: 'flex', alignItems: 'center', gap: 8}}>
                <CheckSquare size={20} color="var(--green)"/> Close Books for {monthLabel}
              </h3>
              <button className="btn-close" onClick={() => setIsClosingOpen(false)}><X size={20} /></button>
            </div>
            
            <form onSubmit={handleCloseBooks} className="add-bill-form">
              <div className="modal-intro" style={{marginBottom: 20, background: 'var(--bg)', padding: 15, borderRadius: 8, fontSize: '0.9rem', color: 'var(--text)'}}>
                <p style={{margin: 0}}>
                  This will finalize the month. Any difference between your <b>Expected</b> cash and <b>Actual</b> cash will be saved as an adjustment for next month.
                </p>
              </div>

              {owners.filter(o => o !== 'Shared').map(owner => {
                const stats = getPersonStats(owner);
                const projected = stats.totalFree;
                return (
                  <div key={owner} style={{marginBottom: 15}}>
                    <div style={{display:'flex', justifyContent:'space-between', marginBottom: 6, fontSize: '0.9rem'}}>
                      <span style={{fontWeight: 600}}>{owner}</span>
                      <span style={{color: 'var(--text-dim)'}}>Expected: ${projected.toFixed(0)}</span>
                    </div>
                    <div className="form-row">
                      <input 
                        ...
                        value={closingBalances[owner] || ''}
                        onChange={(e) => setClosingBalances({...closingBalances, [owner]: parseFloat(e.target.value)})}
                        required
                        autoFocus={owner === owners[1]} // Autofocus first person
                      />
                    </div>
                    {/* Live Diff Preview */}
                    {closingBalances[owner] !== undefined && (
                      <div style={{textAlign: 'right', fontSize: '0.8rem', marginTop: 4, 
                        color: (closingBalances[owner] - projected) >= 0 ? 'var(--green)' : 'var(--red)'}}>
                        Difference: {(closingBalances[owner] - projected) > 0 ? '+' : ''}${(closingBalances[owner] - projected).toFixed(0)}
                      </div>
                    )}
                  </div>
                );
              ...
              <div className="form-actions" style={{justifyContent: 'flex-end', marginTop: 20}}>
                <button type="button" className="btn-cancel" onClick={() => setIsClosingOpen(false)}>Cancel</button>
                <button type="submit" className="btn-primary">Confirm & Close</button>
              </div>
            </form>
          </div>
        </div>
...
// ... (Sub-components: DayPicker, ChecklistWidget, BalanceSection, BillCard remain unchanged) ...
